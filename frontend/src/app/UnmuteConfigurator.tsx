...
export type Instructions =
  ...
  | { type: "rag"; language?: LanguageCode };
  ...
  useEffect(() => {
    const fetchVoicesData = async () => {
      if (backendServerUrl && voices === null) {
        ...
        const defaultVoice =
          englishVoices.find((voice) => voice.name === "Explanation") ||
          englishVoices[Math.floor(Math.random() * englishVoices.length)];

        setConfig({
          ...config,
          voice: defaultVoice.source.path_on_server,
          voiceName: getVoiceName(defaultVoice),
          instructions:
            defaultVoice.instructions || DEFAULT_UNMUTE_CONFIG.instructions,
        });
      }
    };
