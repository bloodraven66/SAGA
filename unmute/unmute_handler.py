...
class UnmuteHandler(AsyncStreamHandler):
    # Pause-prediction (STT-VAD) barge-in threshold: while the bot is speaking, a
    # pause_prediction below this value counts as the user talking -> interrupt.
    # LOWER = stricter = fewer (false) interruptions; subclasses can override.
    VAD_INTERRUPT_PAUSE_THRESHOLD: float = 0.4

    # End-of-turn (pause) threshold: pause_prediction rises the LONGER the user is
    # silent, so this is effectively "how long a pause counts as end-of-turn."
    # HIGHER = requires a longer/more-confident pause = short mid-sentence pauses
    # are NOT mistaken for the user finishing; subclasses can override.
    ...
    async def _on_vad_pause(self) -> None:
        """Called immediately when VAD fires, before STT flush begins.

        Subclasses can override to act on pre-buffered speculative audio
        without waiting for the STT lookahead flush.
        """
    ...
    async def receive(self, frame: tuple[int, np.ndarray]) -> None:
        ...
        if self.stt_end_of_flush_time is None:
            ...
            if self.determine_pause():
                ...
                and stt.pause_prediction.value < self.VAD_INTERRUPT_PAUSE_THRESHOLD
    ...
    def determine_pause(self) -> bool:
        ...
        if stt.pause_prediction.value > self.VAD_PAUSE_THRESHOLD:
    ...
    async def _should_interrupt_on_stt_message(self, data: STTWordMessage) -> bool:
        """Whether a non-empty STT delta while bot_speaking should barge in.

        Extracted as a seam (like _handle_tts_text_message) so subclasses with
        slower turn-startup latency can add a grace period: the STT server's
        own flush can emit a trailing word of the utterance that triggered
        the current turn *after* conversation_state has already flipped to
        "bot_speaking", which reads identically to a genuine new utterance
        starting. The base handler rarely hits this in practice because it
        starts streaming real audio almost immediately; slower generation
        paths make the race far more likely.
        """
        return self.chatbot.conversation_state() == "bot_speaking"
    ...
    async def _stt_loop(self, stt: SpeechToText):
        try:
            async for data in stt:
                ...
                if self.chatbot.conversation_state() == "bot_speaking":
                    if await self._should_interrupt_on_stt_message(data):
                        logger.info("STT-based interruption")
                        await self.interrupt_bot()
                    else:
                        # Suppressed (e.g. a subclass's grace period treating
                        # this as a trailing flush artifact of the SAME
                        # utterance that already triggered the current turn).
                        # Do NOT record it as a new user turn either -- doing
                        # so would append a "user" entry right after the
                        # in-progress turn's empty "assistant" placeholder,
                        # making chat_history look like the user already
                        # started a new utterance mid-response even though no
                        # real interruption happened.
                        continue
    ...
    async def _handle_tts_text_message(
        self, message: TTSTextMessage, generating_message_i: int
    ) -> None:
        """Called from _tts_loop for each TTSTextMessage (one per spoken word).

        Extracted as a seam so subclasses that need to track playback progress
        (e.g. unmute_handler_agentic_rag.py's note-delivery gating) can hook in
        without duplicating the rest of _tts_loop.
        """
        ...
        try:
            ...
            async for message in tts:
                ...
                if isinstance(message, TTSAudioMessage):
                    ...
                    await self._handle_tts_text_message(
                        message, generating_message_i
                    )
                else:
