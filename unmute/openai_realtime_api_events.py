...
class UnmuteAgenticUpdate(BaseEvent[Literal["unmute.agentic.update"]]):
    """Live trace of the agentic RAG loop for the SAGA UI: turn start, spoken
    notes, tool calls (with query + freshness/impact), retrieved papers, the
    grounded answer, and turn end -- pushed as each step happens so the UI can
    animate the agent's cognition in real time.

    `kind` is one of: turn_start, note, tool_calls, tool_results, answer,
    turn_end, interrupted. `data` carries the payload for that kind.
    """
    kind: str
    data: dict[str, Any] = {}
