...
# Must be set before any `unmute.*` import: Chatbot() builds a throwaway default
# system prompt on construction (immediately overwritten by AgenticRagInstructions),
# which calls autoselect_model() -> queries the *Gemma* LLM server just for a
# readable model name. This handler uses Qwen (AGENTIC_LLM_SERVER), not Gemma, so
# short-circuit that probe. Same guard as evaluate_recording_agenticrag.py.
os.environ.setdefault(
    "KYUTAI_LLM_MODEL", os.environ.get("KYUTAI_AGENTIC_LLM_MODEL", "Qwen/Qwen3.5-9B")
)
...
from unmute.unmute_handler_agentic_rag import UnmuteHandlerAgenticRAG
...
@app.get("/")
def root():
    return {"message": "You've reached the Unmute Agentic RAG backend server."}
...
async def _get_health(
    ...
    async with asyncio.TaskGroup() as tg:
        ...
        # The agentic handler's LLM is Qwen (AGENTIC_LLM_SERVER), not Gemma.
        llm_up = tg.create_task(
            asyncio.to_thread(
                _check_server_status,
                _ws_to_http(AGENTIC_LLM_SERVER) + "/v1/models",
            )
        )
        rag_up = tg.create_task(
            asyncio.to_thread(
                _check_server_status,
                SPEECH_RAG_SERVER + "/health",
                headers={"X-API-Key": SPEECH_RAG_API_KEY} if SPEECH_RAG_API_KEY else None,
            )
        )
...
async def websocket_route(websocket: WebSocket):
    ...
    async with SEMAPHORE:
        try:
            ...
            handler = UnmuteHandlerAgenticRAG()
            async with handler:
